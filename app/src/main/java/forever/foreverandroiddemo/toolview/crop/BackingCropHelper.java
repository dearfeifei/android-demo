 ...
 */
public class BackingCropHelper {

    public static final String TAG = "BackingCropHelper";
    ...
    public static void handleResult(BackingCropHandler handler, int requestCode, int resultCode, Intent data) {
        if (handler == null) return;

        if (resultCode == Activity.RESULT_CANCELED) {
            handler.onCancel();
        } else if (resultCode == Activity.RESULT_OK) {
            BackingCropParams cropParams = handler.getBackingCropParams();
            if (cropParams == null) {
            ...
            switch (requestCode) {
                ...
                case REQUEST_CROP:
                    if (isPhotoReallyCropped(cropParams.uri)) {
                        ...
                        Context context = handler.getBackingCropParams().context;
                        if (context != null) {
    ...
    private static void onPhotoCropped(BackingCropHandler handler, BackingCropParams cropParams) {
        if (cropParams.compress) {
            Uri originUri = cropParams.uri;
            Uri compressUri = BackingCropHelper.generateUri();
            BackingCompressImageUtils.compressImageFile(cropParams, originUri, compressUri);
    ...
    public static Intent buildGalleryIntent(BackingCropParams params) {
        Intent intent;
        if (params.enable) {
    ...
    public static Intent buildCameraIntent(BackingCropParams params) {
    ...
    private static Intent buildCropFromUriIntent(BackingCropParams params) {
        return buildCropIntent("com.android.camera.action.CROP", params);
    }

    private static Intent buildCropIntent(String action, BackingCropParams params) {
