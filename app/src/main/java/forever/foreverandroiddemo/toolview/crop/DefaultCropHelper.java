 ...
 */
public class DefaultCropHelper {
    ...
    public static void handleResult(DefaultCropHandler handler, int requestCode, int resultCode, Intent data) {
        if (handler == null) return;

        if (resultCode == Activity.RESULT_CANCELED) {
            handler.onCancel();
        } else if (resultCode == Activity.RESULT_OK) {
            DefaultCropParams cropParams = handler.getCropParams();
            if (cropParams == null) {
    ...
    private static void onPhotoCropped(DefaultCropHandler handler, DefaultCropParams cropParams) {
        if (cropParams.compress) {
            Uri originUri = cropParams.uri;
            Uri compressUri = DefaultCropHelper.generateUri();
            DefaultCompressImageUtils.compressImageFile(cropParams, originUri, compressUri);
    ...
    public static Intent buildGalleryIntent(DefaultCropParams params) {
        Intent intent;
        if (params.enable) {
    ...
    public static Intent buildCameraIntent(DefaultCropParams params) {
    ...
    private static Intent buildCropFromUriIntent(DefaultCropParams params) {
        return buildCropIntent("com.android.camera.action.CROP", params);
    }

    private static Intent buildCropIntent(String action, DefaultCropParams params) {
